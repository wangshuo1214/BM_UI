  ...
  <div class="app-container">
    ...
    <el-row>
      <a style="font-size:5px; font-family: cursive; margin-left: 10px; font-size: 15px;">
        截止目前为止，累计开支<font style="color:cornflowerblue; font-size: 20px; color: red; font-weight: bold">{{ totalSellInfo }}</font>元，
        其中本月开支<font style="color:cornflowerblue;  font-size: 20px; color: red; font-weight: bold">{{ monthSellInfo }}</font>元、
        今日开支<font style="color:cornflowerblue;  font-size: 20px; color: red; font-weight: bold">{{ daySellInfo }}</font>元。
      </a>
    </el-row>
...
<script>
import { listSellRecord, addSellRecord, updateSellRecord, getSellRecord, delSellRecord, getSellInfo } from '@/api/sellRecord'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getSellInfo().then(response => {
        this.totalSellInfo = response.data.totalSellInfo
        this.monthSellInfo = response.data.monthSellInfo
        this.daySellInfo = response.data.daySellInfo
      }
      )
    },
    /** 查询消费对象列表 */
    ...
    handleUpdate(row) {
      ...
      getSellRecord(orderId).then(response => {
